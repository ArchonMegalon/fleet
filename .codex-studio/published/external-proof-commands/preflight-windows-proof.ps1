...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
bash -lc 'if [ -z "${CHUMMER_UI_REPO_ROOT:-}" ] && [ ! -d /docker/chummercomplete/chummer6-ui ]; then echo ''external-proof-ui-repo-root-missing: set CHUMMER_UI_REPO_ROOT to the chummer6-ui checkout root on the proof host'' >&2; exit 1; fi'
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
bash -lc 'if ! command -v bash >/dev/null 2>&1; then echo ''external-proof-bash-missing'' >&2; echo ''Hint: install Git Bash or run the lane from WSL so the generated shell commands can execute.'' >&2; exit 1; fi'
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
exit 0
