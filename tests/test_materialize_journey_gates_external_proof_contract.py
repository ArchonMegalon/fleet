...
def test_external_proof_reasons_fail_close_noncanonical_tuple_id_casing() -> None:
    payload = {
        "status": "published",
        "desktopTupleCoverage": {
            "complete": False,
    ...
    reasons = JOURNEY_GATES_MODULE._release_channel_external_proof_reasons(payload)
    assert any(
        "desktopTupleCoverage.externalProofRequests.tupleId' must be lowercase canonical 'head:rid:platform'"
        in reason
        for reason in reasons
    )
...
def test_external_proof_reasons_fail_close_duplicate_missing_tuple_inventory_rows() -> None:
    ...
    reasons = JOURNEY_GATES_MODULE._release_channel_external_proof_reasons(payload)
    assert any(
        "desktopTupleCoverage.missingRequiredPlatformHeadRidTuples' must not contain duplicate entries." in reason
        for reason in reasons
    )
...
def test_external_proof_reasons_fail_close_noncanonical_and_case_variant_duplicate_missing_tuple_rows() -> None:
    ...
    reasons = JOURNEY_GATES_MODULE._release_channel_external_proof_reasons(payload)
    assert any(
        "desktopTupleCoverage.missingRequiredPlatformHeadRidTuples' must contain lowercase canonical 'head:rid:platform' entries."
        in reason
        for reason in reasons
    )
    assert any(
...
def test_install_journey_blocks_when_support_external_proof_sha256_drift(
    tmp_path: Path,
) -> None:
    registry = tmp_path / "GOLDEN_JOURNEY_RELEASE_GATES.yaml"
    status_plane = tmp_path / "STATUS_PLANE.generated.yaml"
...
""".strip()
    ...
    assert journey["state"] == "blocked"
    assert any(
        "external_proof_request.expected_installer_sha256 must match release-channel tuple truth"
        in reason
        for reason in journey["blocking_reasons"]
    )
...
def test_install_journey_blocks_when_support_case_backed_flag_is_non_boolean(
    tmp_path: Path,
) -> None:
    registry = tmp_path / "GOLDEN_JOURNEY_RELEASE_GATES.yaml"
    status_plane = tmp_path / "STATUS_PLANE.generated.yaml"
...
""".strip()
        ...
        json.dumps(
            {
                ...
                "packets": [
                    {
                        "packet_id": "case-backed-type-drift",
                        "support_case_backed": "false",
                        "install_truth_state": "available",
                        "install_diagnosis": {
                            "registry_channel_id": "stable",
                            "registry_release_channel_status": "published",
                            "registry_release_version": "1.2.3",
                            "registry_release_proof_status": "passed",
                            "external_proof_required": False,
                        },
                        "fix_confirmation": {"state": "fix_verified", "update_required": False},
                        "recovery_path": {"action_id": "none", "href": "/downloads"},
                    }
                ],
            },
            indent=2,
    ...
    assert journey["state"] == "blocked"
    assert any(
        "support packet case-backed-type-drift is missing boolean support_case_backed."
        in reason
        for reason in journey["blocking_reasons"]
    )
    assert int(journey["signals"]["support_install_truth_contract_violation_count"]) > 0
...
def test_install_journey_blocks_when_support_install_truth_predates_release_channel_snapshot(
    tmp_path: Path,
) -> None:
    registry = tmp_path / "GOLDEN_JOURNEY_RELEASE_GATES.yaml"
    status_plane = tmp_path / "STATUS_PLANE.generated.yaml"
...
""".strip()
    ...
    progress_report.write_text(
        json.dumps({"generated_at": "2026-04-04T20:00:00Z", "history_snapshot_count": 1}, indent=2) + "\n",
        encoding="utf-8",
    )
    progress_history.write_text(
        json.dumps({"generated_at": "2026-04-04T20:00:00Z", "snapshot_count": 1}, indent=2) + "\n",
        encoding="utf-8",
    )
    support_packets.write_text(
        json.dumps(
...
def test_install_journey_blocks_when_support_packet_registry_release_version_drifts(
    tmp_path: Path,
) -> None:
    registry = tmp_path / "GOLDEN_JOURNEY_RELEASE_GATES.yaml"
    status_plane = tmp_path / "STATUS_PLANE.generated.yaml"
...
""".strip()
        ...
        json.dumps(
            {
                ...
                "packets": [
                    {
                        "packet_id": "support_packet_registry_drift",
                        "install_truth_state": "ready",
                        "install_diagnosis": {
                            "registry_channel_id": "stable",
                            "registry_release_channel_status": "published",
                            "registry_release_version": "1.2.2",
                            "registry_release_proof_status": "passed",
                            "external_proof_required": False,
                        },
                        "fix_confirmation": {
    ...
    assert journey["state"] == "blocked"
    assert any(
        "support packet support_packet_registry_drift install_diagnosis.registry_release_version must match release-channel value '1.2.3'"
        in reason
        for reason in journey["blocking_reasons"]
    )
