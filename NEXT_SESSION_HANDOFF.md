...
- The active frontier from `chummer-design` is now milestone `4` in W1, with W2 milestone work shifting to `8`, `9`, and `10` after the crew/roster, travel, and Build Lab closeouts.
- Fleet now has a repo-local design-completion supervisor:
  ...
  - The current dry-run derivation now selects milestone `4` as the remaining W1 frontier and leaves milestones `8` through `14` open for later re-derivation.
