...
try:
    from scripts.materialize_next90_m136_fleet_derived_release_health_planes import (
        DEFAULT_OUTPUT,
    ...
    from materialize_next90_m136_fleet_derived_release_health_planes import (  # type: ignore
        DEFAULT_OUTPUT,
        DESIGN_QUEUE_STAGING,
        FLEET_QUEUE_STAGING,
        FLAGSHIP_PRODUCT_BAR,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify the Fleet M136 derived release-health plane packet.")
...
def main() -> int:
    ...
    if not actual:
        ...
        if actual.get("contract_name") != "fleet.next90_m136_derived_release_health_planes":
            issues.append("generated artifact contract_name is missing or unexpected")
        if actual.get("package_id") != PACKAGE_ID:
            issues.append("generated artifact package_id drifted from the assigned Fleet M136.11 package")
        if actual.get("frontier_id") != FRONTIER_ID:
            issues.append("generated artifact frontier_id drifted from the assigned Fleet M136.11 package")
        if actual.get("milestone_id") != MILESTONE_ID:
            issues.append("generated artifact milestone_id drifted from the assigned Fleet M136.11 package")
        if actual.get("work_task_id") != WORK_TASK_ID:
            issues.append("generated artifact work_task_id drifted from the assigned Fleet M136.11 package")
        if _normalized_payload(actual) != _normalized_payload(expected):
            for key, message in (
                ("status", "monitor status drifted from recomputed derived release-health truth"),
                ("canonical_monitors", "canonical monitor sections drifted from the M136.11 contract"),
                ("runtime_monitors", "runtime monitor sections drifted from recomputed derived plane truth"),
                ("monitor_summary", "monitor summary drifted from recomputed derived plane truth"),
                ("package_closeout", "package closeout posture drifted from recomputed derived plane truth"),
    ...
    if args.json:
        ...
        print("M136.11 derived release-health plane verifier failed:", file=sys.stderr)
        for issue in issues:
            print(f" - {issue}", file=sys.stderr)
    else:
        print("M136.11 derived release-health plane verifier passed")
