...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
bash -lc 'python3 -c ''import hashlib, os, pathlib; target_root=pathlib.Path(os.environ[''"''"''TARGET_ROOT''"''"'']); tuple_id=''"''"''avalonia:win-x64:windows''"''"''; relative=''"''"''files/chummer-avalonia-win-x64-installer.exe''"''"''; expected=''"''"''cb3493c1113c23b5e496dfe8a1e6de9afc43c802d7da865adc5255497341e5c4''"''"''; path=target_root / relative; assert path.is_file(), f''"''"''external-proof-bundle-installer-missing:{tuple_id}:{path}''"''"''; digest=hashlib.sha256(path.read_bytes()).hexdigest().lower(); assert digest==expected, f''"''"''installer-contract-mismatch:{tuple_id}:{path}:digest={digest}:expected={expected}''"''"'''''
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
