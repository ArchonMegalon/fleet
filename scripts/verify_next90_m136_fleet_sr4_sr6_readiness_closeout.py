...
try:
    from scripts.materialize_next90_m136_fleet_sr4_sr6_readiness_closeout import (
        DEFAULT_OUTPUT,
    ...
    from materialize_next90_m136_fleet_sr4_sr6_readiness_closeout import (  # type: ignore
        DEFAULT_OUTPUT,
        DESIGN_QUEUE_STAGING,
        FLEET_QUEUE_STAGING,
        FLAGSHIP_PRODUCT_BAR,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify the Fleet M136 SR4/SR6 readiness closeout packet.")
...
def main() -> int:
    ...
    if not actual:
        ...
        if actual.get("contract_name") != "fleet.next90_m136_sr4_sr6_readiness_closeout":
            issues.append("generated artifact contract_name is missing or unexpected")
        if actual.get("package_id") != PACKAGE_ID:
            issues.append("generated artifact package_id drifted from the assigned Fleet M136.19 package")
        if _normalized_payload(actual) != _normalized_payload(expected):
            for key, message in (
                ("status", "monitor status drifted from recomputed SR4/SR6 closeout truth"),
                ("canonical_monitors", "canonical monitor sections drifted from the M136.19 contract"),
                ("runtime_monitors", "runtime monitor sections drifted from recomputed SR4/SR6 closeout truth"),
                ("monitor_summary", "monitor summary drifted from recomputed SR4/SR6 closeout truth"),
                ("package_closeout", "package closeout posture drifted from recomputed SR4/SR6 closeout truth"),
    ...
    if args.json:
        ...
        print("M136.19 SR4/SR6 readiness closeout verifier failed:", file=sys.stderr)
        for issue in issues:
            print(f" - {issue}", file=sys.stderr)
    else:
        print("M136.19 SR4/SR6 readiness closeout verifier passed")
