"""Provisional v3 compatibility, never operational approval or hosted eligibility."""
...
# No genuine Two-Green receipt is pinned for this provisional PR source. Keep
# full-eligibility tests gated; never rewrite the historical 7cef receipt to fit.
...
def current_original(current_source):
    if ORIGINAL_RECEIPT_SHA256 is None:
        pytest.skip("no genuine Two-Green receipt pinned for provisional Android 411e0205")
...
def test_provisional_policy_cannot_issue_before_evidence_or_key_access(monkeypatch):
    class NoKeyAccess(dict):
        def get(self, *args, **kwargs):
            pytest.fail("provisional dormant issuance accessed the key environment")

    def no_sign(*args, **kwargs):
        pytest.fail("provisional dormant issuance reached signing")

    monkeypatch.setattr(approval, "sign_ed25519", no_sign)
    args = argparse.Namespace(policy=fixture.POLICY, **fixture.inputs())
    assert ORIGINAL_RECEIPT_SHA256 is None
...
def test_provisional_commit_cannot_substitute_a_different_main_with_same_tree():
...
def test_provisional_pr_source_must_still_equal_observed_protected_main(tmp_path, monkeypatch):
