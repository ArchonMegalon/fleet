...
def test_statefile_shard_summaries_preserves_waiting_for_model_output_from_stderr_tail(
    monkeypatch, tmp_path: Path
) -> None:
    module = _load_module()
    root = tmp_path
    ...
    try:
        ...
        aggregate_root = root / "state" / "chummer_design_supervisor"
        shard_root = aggregate_root / "shard-1"
        run_dir = shard_root / "runs" / "20260421T041705Z-shard-1"
        run_dir.mkdir(parents=True, exist_ok=True)
        stderr_path = run_dir / "worker.stderr.log"
        stdout_path = run_dir / "worker.stdout.log"
        last_message_path = run_dir / "last_message.txt"
        ...
        module._write_json(
            shard_root / "state.json",
            {
                "updated_at": "2026-04-21T04:20:56Z",
                "mode": "completion_review",
                "frontier_ids": [1239074135],
                "open_milestone_ids": [1239074135],
                "active_run_id": "20260421T041705Z-shard-1",
                "active_run_started_at": "2026-04-21T04:17:05Z",
                "active_run_worker_pid": 61942,
                "active_run_worker_first_output_at": "2026-04-21T04:20:56Z",
                "active_run_worker_last_output_at": "2026-04-21T04:20:56Z",
...
def test_statefile_shard_summaries_waiting_tail_overrides_stale_streaming_progress(
    ...
    try:
        ...
        aggregate_root = root / "state" / "chummer_design_supervisor"
        shard_root = aggregate_root / "shard-1"
        run_dir = shard_root / "runs" / "20260421T051205Z-shard-1"
        run_dir.mkdir(parents=True, exist_ok=True)
        stderr_path = run_dir / "worker.stderr.log"
        stdout_path = run_dir / "worker.stdout.log"
        last_message_path = run_dir / "last_message.txt"
        ...
        module._write_json(
            shard_root / "state.json",
            {
                "updated_at": "2026-04-21T05:12:27Z",
                "mode": "completion_review",
                "frontier_ids": [1239074135],
                "open_milestone_ids": [1239074135],
                "active_run_id": "20260421T051205Z-shard-1",
                "active_run_started_at": "2026-04-21T05:12:05Z",
                "active_run_worker_pid": 194592,
...
def test_statefile_shard_summaries_marks_missing_output_artifacts_instead_of_streaming(
    monkeypatch, tmp_path: Path
) -> None:
    module = _load_module()
    root = tmp_path
    ...
    try:
        ...
        aggregate_root = root / "state" / "chummer_design_supervisor"
        shard_root = aggregate_root / "shard-1"
        run_dir = shard_root / "runs" / "20260421T074641Z-shard-1"
        run_dir.mkdir(parents=True, exist_ok=True)
        missing_stdout = run_dir / "worker.stdout.log"
        missing_stderr = run_dir / "worker.stderr.log"
        missing_last_message = run_dir / "last_message.txt"
        module._write_json(
            shard_root / "state.json",
            {
                "updated_at": "2026-04-21T07:49:29Z",
                "mode": "flagship_product",
                "frontier_ids": [123],
                "open_milestone_ids": [123],
                "active_run_id": "20260421T074641Z-shard-1",
                "active_run_started_at": "2026-04-21T07:46:41Z",
                "active_run_worker_pid": 61942,
                "active_run_worker_first_output_at": "2026-04-21T07:48:48Z",
                "active_run_worker_last_output_at": "2026-04-21T07:48:48Z",
                "active_run_progress_state": "streaming",
                "worker_stderr_path": str(missing_stderr.resolve()),
                "worker_stdout_path": str(missing_stdout.resolve()),
                "worker_last_message_path": str(missing_last_message.resolve()),
                "selected_account_alias": "acct-ea-core-06",
                "selected_model": "ea-coder-hard-batch",
            },
        )
        ...
        assert len(summaries) == 1
        shard = summaries[0]
        assert shard["active_run_progress_state"] == "missing_output_artifacts"
    finally:
        module.DEFAULT_WORKSPACE_ROOT = previous_workspace_root


def test_statefile_shard_summaries_waiting_overrides_stale_streaming_without_output_evidence(
    monkeypatch, tmp_path: Path
) -> None:
    module = _load_module()
    root = tmp_path
    ...
    try:
        ...
        aggregate_root = root / "state" / "chummer_design_supervisor"
        shard_root = aggregate_root / "shard-1"
        run_dir = shard_root / "runs" / "20260421T082702Z-shard-1"
        run_dir.mkdir(parents=True, exist_ok=True)
        missing_stdout = run_dir / "worker.stdout.log"
        missing_stderr = run_dir / "worker.stderr.log"
        missing_last_message = run_dir / "last_message.txt"
        module._write_json(
            shard_root / "state.json",
            {
                "updated_at": "2026-04-21T08:28:09Z",
                "mode": "flagship_product",
                "frontier_ids": [123],
                "open_milestone_ids": [123],
                "active_run_id": "20260421T082702Z-shard-1",
                "active_run_started_at": "2026-04-21T08:27:02Z",
                "active_run_worker_pid": 98637,
                "active_run_worker_first_output_at": "",
                "active_run_worker_last_output_at": "",
                "active_run_progress_state": "streaming",
                "worker_stderr_path": str(missing_stderr.resolve()),
