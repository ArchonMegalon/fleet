...
def _relative(name: str, strip_components: int) -> Path | None:
...
def _extract_zip(archive: Path, destination: Path, strip_components: int) -> None:
    with zipfile.ZipFile(archive) as bundle:
        for member in bundle.infolist():
            ...
            relative = _relative(member.filename, strip_components)
            if relative is None:
                continue
...
def _extract_tar(archive: Path, destination: Path, strip_components: int) -> None:
    with tarfile.open(archive, "r:gz") as bundle:
        for member in bundle:
            relative = _relative(member.name, strip_components)
            if relative is None:
                continue
            target = destination / relative
            if member.isdir():
                ...
                resolved = PurePosixPath(
                    posixpath.normpath(str(PurePosixPath(relative.as_posix()).parent / link))
                )
                ...
                source_relative = _relative(member.linkname, strip_components)
                if source_relative is None or not (destination / source_relative).is_file():
                    raise RuntimeError(f"unsafe archive hardlink: {member.name}")
                target.parent.mkdir(parents=True, exist_ok=True)
                os.link(destination / source_relative, target)
            else:
...
def _install(entry: dict[str, object], scratch: Path) -> None:
    ...
    archive_format = str(entry["format"])
    strip_components = int(entry["strip_components"])
...
def main() -> int:
    ...
    for executable in ("aapt2", "apksigner", "zipalign"):
        path = Path("/opt/android-sdk/build-tools/35.0.0") / executable
        path.chmod(0o755)
    receipt = {
        ...
        "archives": [
            {key: entry[key] for key in ("name", "version", "url", "sha256")}
            for entry in lock["archives"]
        ],
    }
