...
- `chummer.run-services` / `chummer6-hub`: `a7830a76`
...
- `chummer.run-services` / `chummer6-hub`
  - live `main` is now at `a7830a76`; re-derive from that head and keep pushing public/account/operator trust posture, publication continuity, and first-session follow-through until milestones `15`, `18`, and `19` no longer depend on deeper account-only views or single-card detail paths
