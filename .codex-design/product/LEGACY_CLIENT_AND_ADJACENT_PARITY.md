...
| Custom data, XML editing, translator tooling, amend authoring bridge | `EditXmlData`, translator-era XML corpus, custom descriptions/data | master-index now projects custom-data directory counts plus missing/stale/governed custom-data lane posture and explicit custom-data/translator lane receipts from `settings.xml` + enabled overlays, and also projects translator corpus plus bridge posture (`missing`/`stale`/`governed`) with language and overlay counts; end-user custom-data authoring UX and translator replacement workflow remain open | partial | `14` |
...
| Legacy and adjacent import/oracle parity | Chummer4, Chummer5a, Hero Lab, Genesis/CommLink evidence | master-index now projects import-oracle fixture-family coverage, certification receipt posture, explicit adjacent SR6 oracle coverage posture (Genesis/CommLink), and bounded-loss import lane receipts for milestone-17 evidence, and the import parity receipt now fail-closes on explicit `import_oracles` plus `adjacent_oracles` naming for Chummer4/5a/Hero Lab and Genesis/CommLink coverage; full first-class import UX closure remains open | partial | `17` |
