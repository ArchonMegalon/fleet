...
- `runtime.ea.env` is now treated as local mutable runtime state. The tracked bundle source is `runtime.ea.env.example`; copy it to `runtime.ea.env` when bootstrapping a new Fleet checkout or install.
