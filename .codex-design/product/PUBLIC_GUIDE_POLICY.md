...
* Feature and horizon suggestions from the public go to `Chummer6`, the public board, Discord, or other public intake lanes, not to `chummer6-design`.
* Public prioritization, polls, and votes are advisory only.
* The guide optimization lane may audit or draft public guide/article improvements only from approved source packets.
* Accepted guide-optimization recommendations must become upstream `chummer6-design` or public-guide source-registry changes before generated guide output changes.
* The generated public guide must not be hand-edited to accept guide-optimization output.
...
## Public signal and guide optimization posture

`PUBLIC_SIGNAL_FEEDBACK_ROADMAP_BRIDGE.md`, `KATTEB_PUBLIC_GUIDE_OPTIMIZATION_LANE.md`, and `PUBLIC_SIGNAL_TO_CANON_PIPELINE.md` define the public signal/content loop.

Hosted public boards can point users at `/feedback`, `/roadmap`, and `/changelog`, but those pages remain projections from Chummer-owned source.

Guide optimization can improve clarity and findability, but public guide truth still comes from this repo and the approved public-guide registries.
