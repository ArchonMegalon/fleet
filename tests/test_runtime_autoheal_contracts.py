...
class RuntimeAutoHealContractTests(unittest.TestCase):
    ...
    def test_fleet_ooda_timer_prompt_bounds_live_refresh_commands(self) -> None:
        ...
        self.assertIn('keeper_timeout_seconds="${FLEET_OODA_CODEXEA_KEEPER_TIMEOUT_SECONDS:-${FLEET_OODA_CODEX_KEEPER_TIMEOUT_SECONDS:-90}}"', script)
        ...
        self.assertIn('pre-codex keeper start target_active=%s timeout=%ss', script)
        self.assertIn('timeout "${keeper_timeout_seconds}s" python3 scripts/fleet_ooda_keeper.py --once --target-active "$target_shards"', script)
        self.assertIn('>"${current_run}/pre_codex_keeper.stdout.log"', script)
        self.assertIn('2>"${current_run}/pre_codex_keeper.stderr.log"', script)
