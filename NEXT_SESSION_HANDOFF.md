...
- 2026-03-31: the EA hard lane now has a real background batch path, and Fleet’s desktop-first design supervisor is routed onto it instead of waiting on the old 300s foreground hard-stream budget.
  ...
  - the background worker reuses the normal response store, preserves codex governance metadata, and lands terminal receipts into `GET /v1/responses/{response_id}` instead of turning long-running hard work into `upstream_timeout:300s` failures just because the upstream model stayed quiet for too long.
  - `fleet` `scripts/codex-shims/codexea` now honors `CODEXEA_CORE_RESPONSES_PROFILE`, and `scripts/run_chummer_design_supervisor.sh` defaults the design-supervisor `core` lane to `CHUMMER_DESIGN_SUPERVISOR_CORE_RESPONSES_PROFILE=core_batch`; the live `/docker/fleet/runtime.env` now sets that seam so recreated supervisors send `X-EA-Codex-Profile=core_batch` on their hard-lane Codex runs.
  - audit follow-up: `codexea` now also defaults the visible hard model/submode to `ea-coder-hard-batch` / `responses_core_batch` whenever `CODEXEA_CORE_RESPONSES_PROFILE=core_batch`, so the lane stays deterministic even if `/v1/codex/profiles` is temporarily unavailable during startup and live-profile lookup cannot override the default.
  - audit follow-up: EA operator surfaces no longer hide `core_batch`; `app/api/routes/admin_view_models.py` now renders all codex governance lanes, so `/admin/providers` shows both the normal hard coder lane and the batch hard coder lane instead of suppressing the new routing path from operators.
  - live verification after rebuilding `ea-api`: `POST /v1/codex/core-batch` and `POST /v1/responses` with `X-EA-Codex-Profile: core_batch` now both return `202 Accepted` with `background_response: true`, `GET /v1/responses/{response_id}` shows the stored in-progress object, and `stream=true` emits `response.created` + heartbeat `response.in_progress` events instead of an immediate timeout.
  - the EA image rebuild exposed a missing runtime dependency that had been masked in the older container image; `ea/requirements.txt` now explicitly includes `PyYAML>=6.0.0`, which is required because `app/api/routes/responses.py` is imported through `admin_view_models` on API startup.
  - current live supervisor evidence: the fresh shard-1 run `20260331T151744Z` is active with `worker.stderr.log` showing `attempt 1/2 account=lane:core lane=core model=default`, and the live Codex process inside `fleet-design-supervisor` now carries `X-EA-Codex-Profile=core_batch` on its EA responses provider config.
...
- 2026-03-31: Fleet now reaches a trustworthy `complete` state on current repo-local evidence even when the last EA worker receipt dies on an external transport timeout.
  ...
  - `chummer-presentation` queue/package truth was republished, the Linux exit gate was rebuilt on the current Avalonia head, and Fleet’s journey/status/support artifacts were regenerated; the 5 warning-stage golden journeys are now gone and the Linux desktop gate is fresh on the current UI head.
  - `fleet` `scripts/chummer_design_supervisor.py` now synthesizes a supervisor evidence receipt when receipt audit fails only because the latest worker died on an external blocker (`upstream_timeout`, auth/quota/backend outage) while golden journeys, weekly pulse, and the Linux exit gate all pass; ETA now short-circuits to `ready` once completion audit passes instead of staying falsely blocked by the stale last-run error.
  - the Linux gate audit now trusts the scoped source/worktree fingerprint over raw git-head equality, so a proof-only commit that republishes the gate artifact no longer invalidates an otherwise identical Avalonia source snapshot.
  - live verification after restarting `fleet-design-supervisor`: `python3 scripts/chummer_design_supervisor.py status` now reports `mode: complete`, `eta.status: ready`, `completion_audit.status: pass`, `completion_audit.journey_gate_status: pass`, and `completion_audit.linux_gate_status: pass`; all 3 shards settle to `mode=complete` with no active frontier.
  - regression coverage now includes the generated-proof publish-path allowance, queue-source overlay fingerprinting, and evidence-backed synthetic receipt completion path; `python3 -m pytest tests/test_studio_publish_contract.py tests/test_materialize_compile_manifest.py tests/test_materialize_package_compile_overlay.py -q` passes with `41` tests and `python3 -m pytest tests/test_chummer_design_supervisor.py -q` passes with `55` tests.
...
- 2026-03-31: the design supervisor now treats EA core stream-idle timeouts as a recoverable long-slice condition instead of an endless cold-retry loop.
  ...
  - the live Fleet runtime env now pins `CHUMMER_DESIGN_SUPERVISOR_FALLBACK_LANES=repair`, `CHUMMER_DESIGN_SUPERVISOR_STREAM_IDLE_TIMEOUT_MS=900000`, and `CHUMMER_DESIGN_SUPERVISOR_STREAM_MAX_RETRIES=8` so restarts actually carry the timeout recovery posture.
  - regression coverage now includes a direct-lane zero-exit `upstream_timeout` fallback test in `tests/test_chummer_design_supervisor.py`.

- 2026-03-31: the Linux desktop hard exit gate now scopes proof identity to actual desktop-gate inputs, so concurrent unrelated repo edits no longer invalidate a clean Avalonia proof.
  - `chummer-presentation` `scripts/materialize-linux-desktop-exit-gate.sh` now fingerprints and snapshots only the Linux-gate input set (`Chummer.Avalonia`, `Chummer.Desktop.Runtime`, `Chummer.Desktop.Runtime.Tests`, `Chummer.Tests`, `Chummer.Presentation`, and the packaging/build helper scripts plus root build props) instead of the whole repo, while still falling back safely if that scoped set is absent.
  - the source-snapshot manifest now records both the initial and finish fingerprints plus entry counts, and only ignores expected generated output directories (`bin`, `obj`, `TestResults`) when sealing the snapshot; unexpected new source-like files still count as tampering.
  - `fleet` `scripts/chummer_design_supervisor.py` now audits those scoped fingerprints directly and exposes both `linux_gate_snapshot_sha` and `linux_gate_snapshot_finish_sha` plus the `linux_gate_snapshot_stable` flag in status/trace output.
  - Fleet regression coverage now includes an unrelated-repo-change pass case plus the snapshot-instability failure case, and `python3 -m pytest tests/test_chummer_design_supervisor.py` now passes with `42` tests.
  - verified live via a fresh `bash scripts/materialize-linux-desktop-exit-gate.sh` run in `chummer-presentation` and a direct Fleet audit pass against the regenerated proof at `2026-03-31T10:09:36Z`; the current canonical proof reports `source_snapshot_entry_count=375`, `source_snapshot_finish_entry_count=375`, and `source_snapshot_identity_stable=true`.

- 2026-03-31: the Linux desktop hard exit gate now runs from an immutable source snapshot and Fleet verifies preserved pre-purge installer artifacts instead of trusting a mutable checkout or self-reported JSON alone.
  - `chummer-presentation` `scripts/materialize-linux-desktop-exit-gate.sh` now fingerprints the actual repo-local file contents, copies the current worktree into a temporary sibling snapshot under `/docker/chummercomplete`, runs tests/build/package/smoke from that snapshot, and publishes proof only when the source fingerprint stays stable start-to-finish.
  - `chummer-presentation` `scripts/run-desktop-startup-smoke.sh` now preserves the installed Linux binary, wrapper, and desktop-entry under the gate run-root before purge, and the install-verification sidecar records both those capture paths and their digests/content.
  - `fleet` `scripts/chummer_design_supervisor.py` now hashes the real worktree contents instead of `git status` text, requires `source_snapshot.mode=filesystem_copy`, requires the snapshot fingerprint to match the proof git snapshots and current worktree fingerprint, requires `stage=complete`, `fallback_package_kind=archive`, `unit_tests.framework=net10.0`, exact artifact digests, and validates the preserved wrapper/desktop-entry capture files directly.
  - Fleet regression coverage now includes missing source-snapshot and wrong-framework failures, and the fixture worktree fingerprint matches the new live content-addressed fingerprint contract.
  - verified via `python3 -m pytest tests/test_chummer_design_supervisor.py` (`40` passing), `bash scripts/materialize-linux-desktop-exit-gate.sh` in `chummer-presentation`, and a direct Fleet audit pass against `/docker/chummercomplete/chummer-presentation/.codex-studio/published/UI_LINUX_DESKTOP_EXIT_GATE.generated.json`; the current canonical proof was regenerated at `2026-03-31T09:57:02Z` and now reports `source_snapshot_mode=filesystem_copy`, `primary_install_verification_status=passed`, and `unit_test_status=passed`.

- 2026-03-31: the Linux `.deb` hard exit gate now proves a real install/remove cycle instead of only unpacking the package.
  - `chummer-presentation` `scripts/run-desktop-startup-smoke.sh` now installs the primary `.deb` into an isolated rootless dpkg admin dir, boots the installed `Chummer.Avalonia` binary from that installed path, purges the package afterward, and persists `install-verification-*.json` plus a `dpkg-*.log` sidecar beside the installer receipt.
  - `fleet` `scripts/chummer_design_supervisor.py` now rejects the Linux gate unless the installer receipt carries `artifactInstallMode=dpkg_rootless_install`, the install-verification sidecar stays under the gate run-root, the dpkg log proves `install` plus `remove`/`status not-installed`, and the receipt/sidecar agree on package name, arch, installed paths, and post-purge disappearance.
  - `chummer-presentation` compliance guards now require the new isolated dpkg install/purge path instead of the older `dpkg-deb -x` shortcut, and the release docs now describe the `.deb` install/remove smoke explicitly.
  - verified via `python3 -m pytest tests/test_chummer_design_supervisor.py` (`38` passing), a direct `bash scripts/run-desktop-startup-smoke.sh ...installer.deb...` smoke pass, and a fresh `bash scripts/materialize-linux-desktop-exit-gate.sh` run; the current canonical proof was regenerated at `2026-03-31T09:39:12Z` and Fleet now reports `primary_install_verification_status=passed`.
  - the earlier raw `dotnet test Chummer.Tests/Chummer.Tests.csproj --filter "Desktop_download_matrix_includes_avalonia_and_blazor_desktop_artifacts"` observation is not a real current repo blocker; the supported `scripts/ai/build.sh` / gate flows succeed because they bootstrap the local compatibility tree correctly, while the raw direct `dotnet test` path bypasses that repo-local package-plane setup.

- 2026-03-31: the Linux desktop exit gate is now hardened against repo-mutation races and proof spoofing, not just stale proofs.
  - `chummer-presentation` `scripts/materialize-linux-desktop-exit-gate.sh` now captures repo git identity at both the start and finish of the gate run, refuses to publish a passing proof if the tracked UI worktree changes mid-run, and records stronger proof metadata for the binary/archive/installer digests plus the expected readiness checkpoint.
  - `fleet` `scripts/chummer_design_supervisor.py` now requires the proof run-root and every artifact/receipt/TRX path to stay under the canonical `.codex-studio/out/linux-desktop-exit-gate` tree, re-hashes the real Linux artifacts, validates startup-smoke receipts against the exact package digest / channel / version / launch target / checkpoint, and verifies the TRX still targets `Chummer.Desktop.Runtime.Tests.dll` instead of trusting counters alone.
  - Fleet regression coverage now includes path-escape, receipt-digest, wrong-assembly, and repo-mutated-during-run failures in `tests/test_chummer_design_supervisor.py`.
  - verified via `python3 -m pytest tests/test_chummer_design_supervisor.py` (`36` passing) plus a fresh live `bash scripts/materialize-linux-desktop-exit-gate.sh` run in `chummer-presentation`; the current canonical proof was regenerated at `2026-03-31T09:27:53Z` and passes the stricter Fleet audit.

- 2026-03-31: Fleet completion now has a hard Linux desktop exit gate, not just receipt/journey/pulse gates.
  - `chummer6-ui` now owns `scripts/materialize-linux-desktop-exit-gate.sh`, which builds the Linux Avalonia binary, emits the primary `.deb` plus fallback archive, runs startup smoke on both packaged outputs, runs `Chummer.Desktop.Runtime.Tests`, and publishes `.codex-studio/published/UI_LINUX_DESKTOP_EXIT_GATE.generated.json`.
  - the materializer now uses a per-run output root plus atomic proof publish, records the UI repo git head / tracked-diff fingerprint, and hard-locks the flagship Avalonia head so stale proofs, fallback-head proofs, and concurrent refresh races no longer satisfy completion.
  - `scripts/run-desktop-startup-smoke.sh` now creates writable bundle/XDG home roots before launching the packaged binary, so Linux startup smoke works in the Fleet container instead of failing on unset `DOTNET_BUNDLE_EXTRACT_BASE_DIR` or unwritable `/.local`.
  - `fleet` `scripts/chummer_design_supervisor.py` now refuses `complete` unless that Linux desktop exit-gate proof passes, and `completion_review` prompts surface the gate explicitly under `Linux desktop exit-gate gaps`.
  - verified live via `python3 -m pytest tests/test_chummer_design_supervisor.py` in `/docker/fleet` and `docker exec fleet-design-supervisor bash -lc 'cd /docker/chummercomplete/chummer6-ui && bash scripts/materialize-linux-desktop-exit-gate.sh'`, with the current proof generated at `2026-03-31T08:52:35Z`.
...
- 2026-03-30: the design supervisor can now run directly on the EA `core` hard-coder lane instead of being trapped behind the protected local Codex account pool.
  ...
  - The local `runtime.env` now points the supervisor at `/docker/fleet/scripts/codex-shims/codexea` with `CHUMMER_DESIGN_SUPERVISOR_WORKER_LANE=core`, `CHUMMER_DESIGN_SUPERVISOR_ACCOUNT_OWNER_IDS=` blank, and `CHUMMER_DESIGN_SUPERVISOR_PARALLEL_SHARDS=3`, so the live supervisor uses only the EA / 1min core lane and no longer carries the `tibor.girschele` / `the.girscheles` / `archon.megalon` owner filter into the loop process.
...
- 2026-03-30: Fleet now supports a first-class `desktop_client` steering profile for the Chummer design supervisor, and the live runtime is pinned to it.
  ...
  - The local `runtime.env` now carries desktop-first steering as explicit text focus plus shard owner groups (`chummer6-ui` / `chummer6-ui-kit`, `chummer6-core`, `chummer6-hub` / `chummer6-hub-registry` / `chummer6-design`) so multiple EA core hard-coder lanes can work in parallel without falling back to the protected human-owner pool.
