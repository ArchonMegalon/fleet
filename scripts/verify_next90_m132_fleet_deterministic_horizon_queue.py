...
try:
    from scripts.materialize_next90_m132_fleet_deterministic_horizon_queue import (
        DEFAULT_OUTPUT,
    ...
    from materialize_next90_m132_fleet_deterministic_horizon_queue import (  # type: ignore
        DEFAULT_OUTPUT,
        DESIGN_QUEUE_STAGING,
        HORIZON_REGISTRY,
        NEXT90_GUIDE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify the Fleet M132 deterministic horizon queue guard packet.")
...
def main() -> int:
    ...
    if not actual:
        ...
        if actual.get("contract_name") != "fleet.next90_m132_deterministic_horizon_queue_guard":
            issues.append("generated artifact contract_name is missing or unexpected")
        if actual.get("package_id") != "next90-m132-fleet-schedule-deterministic-horizon-slices-only-after-owner-h":
            issues.append("generated artifact package_id drifted from the assigned Fleet M132 package")
        if _normalized_payload(actual) != _normalized_payload(expected):
            for key, message in (
                ("status", "monitor status drifted from recomputed deterministic horizon truth"),
                ("canonical_alignment", "canonical alignment drifted from queue and registry truth"),
                ("canonical_monitors", "canonical monitor sections drifted from design-owned deterministic horizon truth"),
                ("queue_gate_monitor", "queue gate monitor drifted from recomputed deterministic horizon truth"),
                ("package_closeout", "package closeout posture drifted from recomputed deterministic horizon truth"),
                ("source_inputs", "source input links drifted from recomputed source truth"),
                ("milestone_id", "milestone_id drifted from the assigned Fleet M132 package"),
                ("frontier_id", "frontier_id drifted from the assigned Fleet M132 package"),
            ):
    ...
    if args.json:
        ...
        print("M132 deterministic horizon queue verifier failed:", file=sys.stderr)
        for issue in issues:
            print(f" - {issue}", file=sys.stderr)
    else:
        print("M132 deterministic horizon queue verifier passed")
