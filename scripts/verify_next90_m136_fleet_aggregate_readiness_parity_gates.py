...
try:
    from scripts.materialize_next90_m136_fleet_aggregate_readiness_parity_gates import (
        CAMPAIGN_CONTINUITY_LIVENESS,
        DEFAULT_OUTPUT,
    ...
    from materialize_next90_m136_fleet_aggregate_readiness_parity_gates import (  # type: ignore
        CAMPAIGN_CONTINUITY_LIVENESS,
        DEFAULT_OUTPUT,
        DESIGN_QUEUE_STAGING,
        FLAGSHIP_PRODUCT_READINESS,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify the Fleet M136 aggregate-readiness parity gate packet.")
...
def main() -> int:
    ...
    if not actual:
        ...
        if actual.get("package_id") != PACKAGE_ID:
            issues.append("generated artifact package_id drifted from the assigned Fleet M136 package")
        if _normalized_payload(actual) != _normalized_payload(expected):
            for key, message in (
                ("status", "monitor status drifted from recomputed aggregate-readiness parity truth"),
                ("canonical_monitors", "canonical monitor sections drifted from the M136 contract"),
                ("runtime_monitors", "runtime monitor sections drifted from recomputed parity/continuity truth"),
                ("monitor_summary", "monitor summary drifted from recomputed parity/continuity truth"),
                ("package_closeout", "package closeout posture drifted from recomputed parity/continuity truth"),
                ("source_inputs", "source input links drifted from recomputed source truth"),
                ("frontier_id", "frontier_id drifted from the assigned Fleet M136 package"),
                ("milestone_id", "milestone_id drifted from the assigned Fleet M136 package"),
            ):
    ...
    if args.json:
        ...
        print("M136 aggregate-readiness parity gate verifier failed:", file=sys.stderr)
        for issue in issues:
            print(f" - {issue}", file=sys.stderr)
    else:
        print("M136 aggregate-readiness parity gate verifier passed")
