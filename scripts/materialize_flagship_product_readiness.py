...
DEFAULT_M140_PORTABILITY_AND_CADENCE_CLOSEOUT_GATE = (
    ROOT / ".codex-studio" / "published" / "NEXT90_M140_FLEET_PORTABILITY_AND_CADENCE_CLOSEOUT_GATES.generated.json"
)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--m140-portability-and-cadence-closeout-gate",
        default=str(DEFAULT_M140_PORTABILITY_AND_CADENCE_CLOSEOUT_GATE),
        help="path to NEXT90_M140_FLEET_PORTABILITY_AND_CADENCE_CLOSEOUT_GATES.generated.json",
    )
    parser.add_argument(
...
def _m138_hero_path_closeout_gate_audit(payload: Dict[str, Any]) -> Dict[str, Any]:
    reasons: List[str] = []
    if not payload:
        reasons.append("M138 hero-path closeout gate is missing.")
        return {
            "ready": False,
            "status": "",
            "hero_path_closeout_status": "",
            "generated_at": "",
            "reasons": reasons,
        }
    status = str(payload.get("status") or "").strip().lower()
    monitor_summary = dict(payload.get("monitor_summary") or {})
    hero_path_closeout_status = str(monitor_summary.get("hero_path_closeout_status") or "").strip().lower()
    runtime_blockers = [
        str(item).strip()
        for item in (monitor_summary.get("runtime_blockers") or [])
        if str(item).strip()
    ...
    generated_at = str(payload.get("generated_at") or payload.get("generatedAt") or "").strip()
    if status != "pass":
        reasons.append("M138 hero-path closeout gate package is not passing.")
    if hero_path_closeout_status not in {"pass", "ready"} and (
        hero_path_closeout_status != "warning" or runtime_blockers
    ):
        reasons.append("M138 hero-path closeout gate still reports blocked runtime proof.")
    if not generated_at:
        reasons.append("M138 hero-path closeout gate generated_at is missing.")
    return {
        "ready": not reasons,
        "status": status,
...
def _m140_portability_and_cadence_closeout_gate_audit(payload: Dict[str, Any]) -> Dict[str, Any]:
    reasons: List[str] = []
    if not payload:
        reasons.append("M140 portability/cadence closeout gate is missing.")
        return {
            "ready": False,
            "status": "",
            "portability_and_cadence_closeout_status": "",
            "generated_at": "",
            "reasons": reasons,
        }
    status = str(payload.get("status") or "").strip().lower()
    monitor_summary = dict(payload.get("monitor_summary") or {})
    portability_and_cadence_closeout_status = str(
        monitor_summary.get("portability_and_cadence_closeout_status") or ""
    ).strip().lower()
    runtime_blockers = [
        str(item).strip()
        for item in (monitor_summary.get("runtime_blockers") or [])
    ...
    generated_at = str(payload.get("generated_at") or payload.get("generatedAt") or "").strip()
    if status != "pass":
        reasons.append("M140 portability/cadence closeout gate package is not passing.")
    if portability_and_cadence_closeout_status not in {"pass", "ready"} and (
        portability_and_cadence_closeout_status != "warning" or runtime_blockers
    ):
        reasons.append("M140 portability/cadence closeout gate still reports blocked runtime proof.")
    if not generated_at:
        reasons.append("M140 portability/cadence closeout gate generated_at is missing.")
    return {
        "ready": not reasons,
        "status": status,
...
def _m141_import_route_closeout_gate_audit(payload: Dict[str, Any]) -> Dict[str, Any]:
    reasons: List[str] = []
    if not payload:
        reasons.append("M141 import-route closeout gate is missing.")
        return {
            "ready": False,
            "status": "",
            "import_route_closeout_status": "",
            "generated_at": "",
            "reasons": reasons,
        }
    status = str(payload.get("status") or "").strip().lower()
    monitor_summary = dict(payload.get("monitor_summary") or {})
    import_route_closeout_status = str(monitor_summary.get("import_route_closeout_status") or "").strip().lower()
    runtime_blockers = [
        str(item).strip()
        for item in (monitor_summary.get("runtime_blockers") or [])
        if str(item).strip()
    ...
    generated_at = str(payload.get("generated_at") or payload.get("generatedAt") or "").strip()
    if status != "pass":
        reasons.append("M141 import-route closeout gate package is not passing.")
    if import_route_closeout_status not in {"pass", "ready"} and (
        import_route_closeout_status != "warning" or runtime_blockers
    ):
        reasons.append("M141 import-route closeout gate still reports blocked runtime proof.")
    if not generated_at:
        reasons.append("M141 import-route closeout gate generated_at is missing.")
    return {
        "ready": not reasons,
        "status": status,
...
def build_flagship_product_readiness_payload(
    *,
    ...
    m140_portability_and_cadence_closeout_gate = load_json(m140_portability_and_cadence_closeout_gate_path)
    m140_portability_and_cadence_closeout_gate_audit = _m140_portability_and_cadence_closeout_gate_audit(
        m140_portability_and_cadence_closeout_gate
    )
    ...
    m143_route_local_output_closeout_gate = load_json(m143_route_local_output_closeout_gate_path)
    m143_route_local_output_closeout_gate_audit = _m143_route_local_output_closeout_gate_audit(
        m143_route_local_output_closeout_gate
    )
    ...
    if not bool(m140_portability_and_cadence_closeout_gate_audit.get("ready")):
    ...
    if not bool(m143_route_local_output_closeout_gate_audit.get("ready")):
        flagship_plane_reasons.append("M143 route-local output closeout gate is not ready.")
    flagship_plane_status, flagship_plane = _coverage_entry(
        positives=(
            ...
            + int(bool(m143_route_local_output_closeout_gate_audit.get("ready")))
        ),
        ...
        evidence={
            ...
            "m138_hero_path_closeout_gate_path": str(m138_hero_path_closeout_gate_path),
            "m138_hero_path_closeout_gate_ready": bool(m138_hero_path_closeout_gate_audit.get("ready")),
            "m138_hero_path_closeout_gate_status": str(m138_hero_path_closeout_gate_audit.get("status") or ""),
            "m138_hero_path_closeout_monitor_status": str(
                m138_hero_path_closeout_gate_audit.get("hero_path_closeout_status") or ""
            ),
            "m138_hero_path_closeout_gate_generated_at": str(
            ...
            "m140_portability_and_cadence_closeout_gate_path": str(m140_portability_and_cadence_closeout_gate_path),
            "m140_portability_and_cadence_closeout_gate_ready": bool(
                m140_portability_and_cadence_closeout_gate_audit.get("ready")
            ),
            "m140_portability_and_cadence_closeout_gate_status": str(
                m140_portability_and_cadence_closeout_gate_audit.get("status") or ""
            ),
            "m140_portability_and_cadence_closeout_monitor_status": str(
                m140_portability_and_cadence_closeout_gate_audit.get("portability_and_cadence_closeout_status") or ""
            ),
            "m140_portability_and_cadence_closeout_gate_generated_at": str(
                m140_portability_and_cadence_closeout_gate_audit.get("generated_at") or ""
            ),
            "m140_portability_and_cadence_closeout_gate_reasons": list(
                m140_portability_and_cadence_closeout_gate_audit.get("reasons") or []
            ),
            "m141_import_route_closeout_gate_path": str(m141_import_route_closeout_gate_path),
            "m141_import_route_closeout_gate_ready": bool(m141_import_route_closeout_gate_audit.get("ready")),
            "m141_import_route_closeout_gate_status": str(m141_import_route_closeout_gate_audit.get("status") or ""),
            "m141_import_route_closeout_monitor_status": str(
                m141_import_route_closeout_gate_audit.get("import_route_closeout_status") or ""
            ),
            "m141_import_route_closeout_gate_generated_at": str(
            ...
            "m143_route_local_output_closeout_gate_path": str(m143_route_local_output_closeout_gate_path),
            "m143_route_local_output_closeout_gate_ready": bool(m143_route_local_output_closeout_gate_audit.get("ready")),
            "m143_route_local_output_closeout_gate_status": str(
                m143_route_local_output_closeout_gate_audit.get("status") or ""
            ),
            "m143_route_local_output_closeout_monitor_status": str(
                m143_route_local_output_closeout_gate_audit.get("route_local_output_closeout_status") or ""
            ),
            "m143_route_local_output_closeout_gate_generated_at": str(
                m143_route_local_output_closeout_gate_audit.get("generated_at") or ""
            ),
            "m143_route_local_output_closeout_gate_reasons": list(
                m143_route_local_output_closeout_gate_audit.get("reasons") or []
            ),
...
def materialize_flagship_product_readiness(
    *,
    ...
    payload = build_flagship_product_readiness_payload(
        ...
        m143_route_local_output_closeout_gate_path=m143_route_local_output_closeout_gate_path,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    payload = materialize_flagship_product_readiness(
        ...
        m143_route_local_output_closeout_gate_path=Path(args.m143_route_local_output_closeout_gate).resolve(),
