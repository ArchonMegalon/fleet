(function () {
  ...
  function resolveJourneyGates() {
    return (state.cockpitStatus && state.cockpitStatus.journey_gates) || resolvePublicStatus().journey_gates || { summary: {}, journeys: [] };
  }
  ...
  function postureSummary() {
    ...
    const journeySummary = resolveJourneyGates().summary || {};
    ...
    if (/emergency|critical/.test(String(summary.scheduler_posture || "").toLowerCase()) || (num(summary.open_incidents, 0) > 0 && num(summary.blocked_groups, 0) > 0) || first(runtimeHealing.alert_state) === "action_needed" || first(publishReadiness.state) === "blocked" || first(journeySummary.overall_state) === "blocked") {
      return {
        state: "frozen",
        label: "Frozen",
    ...
    if (compileLag || num(summary.open_incidents, 0) > 0 || num(supportSummary.closure_waiting_on_release_truth, 0) > 0 || routeReviewsDue > 0 || num(runtimeHealing.degraded_service_count, 0) > 0 || first(publishReadiness.state) === "warning" || first(journeySummary.overall_state) === "warning") {
      return {
        state: "warn",
        label: "Degraded",
  ...
  function buildAttentionItems() {
    ...
    if (first(journeySummary.overall_state) === "blocked") {
      items.push({
        title: "Golden journey proof is blocked",
        detail: `${num(journeySummary.blocked_count, 0)} release-critical journeys are below minimum evidence or posture.`,
        owner: "Fleet journey gate",
        action: first(journeySummary.recommended_action, "Repair the blocked journey before widening release claims."),
        state: "bad",
      });
    } else if (first(journeySummary.overall_state) === "warning") {
      items.push({
        title: "Golden journey proof is not boring yet",
        detail: `${num(journeySummary.warning_count, 0)} journeys still sit below target posture or history depth.`,
        owner: "Fleet journey gate",
        action: first(journeySummary.recommended_action, "Close the remaining journey warning posture before calling the campaign OS steady."),
        state: "warn",
      });
    }
  ...
  function buildHousekeepingItems() {
    ...
    if (first(journeySummary.overall_state) === "blocked" || first(journeySummary.overall_state) === "warning") {
      items.push({
        title: "Golden journey gate review",
        why: "Release posture should be backed by lived end-to-end journey proof, not just repo and compile truth.",
        owner: "Product Governor",
        due: first(journeySummary.overall_state, "warning"),
        action: first(journeySummary.recommended_action, "Review the journey gate warnings and publish the bounded reason."),
        state: first(journeySummary.overall_state) === "blocked" ? "bad" : "warn",
      });
    }
